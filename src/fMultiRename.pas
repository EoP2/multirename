...
uses
  ...
  uFormCommands,        // cm_xxx 命令分发系统（actExecute 用，原样带走）
  DCStringHashListUtf8, // TStringHashListUtf8（原样带走）
  uMRFile,              // 替代 uFile + uFileSource（TFile/TFiles）
  uMRConfig,            // 替代 DC 的 uGlobs 全局配置（uGlobs.pas 已整体移除）
  uMRApp,               // 独立版应用名/配置目录
...
begin
  ...
  // 保存路径范围分隔符（已在 fSelectPathRange 内部直接读写 MRConfig，无需在此同步）
  MRConfig.Save;
