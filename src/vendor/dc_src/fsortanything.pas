...
uses
  ...
  //独立版：窗口状态改用 MRConfig 统一持久化（替代原 uGlobs/uClassesEx）
  uMRConfig;
...
begin
  if MRConfig.SortAnythingWidth  > 0 then Width  := MRConfig.SortAnythingWidth;
  if MRConfig.SortAnythingHeight > 0 then Height := MRConfig.SortAnythingHeight;
  if MRConfig.SortAnythingLeft   > 0 then Left   := MRConfig.SortAnythingLeft;
  if MRConfig.SortAnythingTop    > 0 then Top    := MRConfig.SortAnythingTop;
end;

{ TfrmSortAnything.SaveWindowState }
procedure TfrmSortAnything.SaveWindowState;
begin
  MRConfig.SortAnythingLeft   := Left;
  MRConfig.SortAnythingTop    := Top;
  MRConfig.SortAnythingWidth  := Width;
  MRConfig.SortAnythingHeight := Height;
end;
